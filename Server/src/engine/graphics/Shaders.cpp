...
namespace Server::Engine::Graphics {
	...
	ShaderProgram::ShaderProgram(const std::string& vertexShaderFilepath, const std::string& fragmentShaderFilepath) {
		...
		if(vertexShader == 0) {
			SERVER_ERROR("Failed to compile vertex shader");
		...
		if(linkingStatus != GL_TRUE) {
			SERVER_ERROR("Failed to link shader program. Error: %s", shaderProgramLog.data());
			m_HadErrorBuilding = true;
			return;
		}
	}
	...
	GLuint ShaderProgram::CompileShaderSource(const std::string& shaderSource, GLenum shaderType) {
		...
		if(compilationStatus != GL_TRUE) {
			...
			SERVER_ERROR("Failed to compile shader source. Error: %s", shaderLog.data());
			return 0;
		}
	...
	void ShaderProgram::SetUniformVec4Array(GLint uniformLocation, const std::vector<glm::vec4>& vectors) const {
		glUniform4fv(uniformLocation, GLsizei(vectors.size()), glm::value_ptr(vectors[0]));
	}

	void ShaderProgram::SetUniformMatrix4Array(GLint uniformLocation, const std::vector<glm::mat4>& matrices) const {
		glUniformMatrix4fv(uniformLocation, GLsizei(matrices.size()), GL_FALSE, glm::value_ptr(matrices[0]));
	}
